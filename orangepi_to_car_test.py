...
本脚本只发送 SR5E1E3_CHASSIS_DEBUG_GUIDE(1).md 中已经写明的核心串口帧。
...
class CarMovementTestNode(Node):
    def __init__(self, port, baud):
        super().__init__("orangepi_to_car_test")
        self.fd = None
...
def main(argv=None):
    ...
    try:
        node = CarMovementTestNode(parsed_args.port, parsed_args.baud)
        rclpy.spin(node)
