...
> 决定：用独立无线路由器建局域网，车与飞车都作为**普通客户端（STA）连同一路由器**。
> 影响：车端**不再发 AP** → 本文「二 `wifi_ap_manager`」「三 单网卡 AP→STA 坑」「四 标志位机制」**全部作废**；车板退化为"连路由器 wifi"的普通客户端，开机靠 nmcli autoconnect 即可。
> 新方案 TODO 见飞车端文档「七、路由器新方案」。以下旧内容仅作历史记录与避坑参考。

本文记录（旧）车端 AP 方案。飞车端对应文档见 `../../fly_car/docs/wifi_lan_autostart.md`。
